...
class Solution:
    def findWords_0(self, board: List[List[str]], words: List[str]) -> List[str]:
        def find(y, x, word):
            n = len(board)
            m = len(board[0])
            visited = set()
            ...
            def dfs(y, x, id):
                ...
                for shiftY, shiftX in shifts:
                    i = y + shiftY
                    j = x + shiftX
                    if (i, j) in visited or isInvalidLoc(i, j):
                        continue
                    visited.add((i, j))
    ...
    def findWords(self, board: List[List[str]], words: List[str]) -> List[str]:
        ...
        def dfs(y, x, node, word):
            nonlocal shifts, trie
...
def test ():
    params = [
        ...
        {
            'input': [[["a","b","c","e"],["x","x","c","d"],["x","x","b","a"]], ["abc","abcd"]],
            'output': ["abc","abcd"],
        },
        # {
        #     'input': [

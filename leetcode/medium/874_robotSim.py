...
class Solution:
    def robotSim(self, commands: List[int], obstacles: List[List[int]]) -> int:
        obstaclesSet = set([(y, x) for x,y in obstacles])
        directions = [
            [1, 0], # top
            [0, 1], # right
            [-1, 0], # bottom
        ...
        for com in commands:
            if com < 0:
                ...
                for i in range(com):
                    newY = loc[0] + directions[d][0]
                    newX = loc[1] + directions[d][1]

                    if (newY, newX) in obstaclesSet:
                        break

                    loc = [newY, newX]
                    r = max(r, (newY**2 + newX**2))
...
def test ():
    ...
    for param in params:
        commands, obstacles = param['input']

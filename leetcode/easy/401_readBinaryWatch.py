...
class Solution_0:
    def readBinaryWatch(self, turnedOn: int) -> List[str]:
        ...
        def dfs(hId, mId, hSum, mSum, vSum):
            nonlocal turnedOn, res

            if (hId, mId, hSum, mSum, vSum) in visited:
                return

            if vSum > turnedOn:
                return
            if hSum > 11 or mSum > 59:
                return

            if vSum == turnedOn:
                h = str(hSum)
                m = '0' + str(mSum) if mSum < 10 else str(mSum)
                res.add(h + ':' + m)
                return

            visited.add((hId, mId, hSum, mSum, vSum))

            if hId < hLen:
                dfs(hId+1, mId, hSum + hours[hId], mSum, vSum+1)
                dfs(hId+1, mId, hSum, mSum, vSum)

            if mId < mLen:
                dfs(hId, mId+1, hSum, mSum + minutes[mId], vSum+1)
                dfs(hId, mId+1, hSum, mSum, vSum)
